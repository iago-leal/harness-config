...
- **Linguagens Principais (aplicação):**
  - **Python (`.py`)**: **90 arquivos** — **56** em `.harness/harness-core/src/` e **34** em `.harness/harness-core/tests/` (33 `test_*.py` + `helpers.py`). Acréscimo desde a 009: `src/core/migrate/{__init__,service}.py` (020), `src/core/session/{close_flow,resume_context}.py` (018/021), e os testes correspondentes `test_migrate.py`, `test_close_flow.py`, `test_resume_context.py`, `test_session_sinks.py`, `test_session_skills.py`, `test_skill_scripts.py`, `test_shim.py`, `test_local_apply.py`, `test_git_dirty.py`, `test_gitignore_entry.py`, `test_regen.py`, `test_offers.py`, `test_wrapper.py`, `test_footprint.py`, `test_harness_profiles.py`, `test_install_claude_settings.py` (o crescimento reflete as features 010-021, não só uma).
  - **Markdown (`.md`)**: ~18 arquivos no escopo de aplicação — instruções de agente (`CLAUDE.md`, `GEMINI.md`, `AGENTS.md`), as **12 fichas de decisão** em `.harness/decisoes/` (`MD-0001`..`MD-0012`, cresceu de 5 para 12) mais o `_cabecalho.md`, o índice `.harness/microdecisoes.md`, o estado de sessão (`.harness/estado-da-sessao.md`) e o brief solto na raiz `BRIEF-oferta-commit-pendente-ao-encerrar.md` (registro de intenção pré-feature, não normativo).
...
- **`src/core/`** — Regras de negócio (domínio puro), uma pasta por capacidade:
  ...
  - **`decisions/`** 🟢 — carga, validação de integridade do grafo e compilação do índice de microdecisões (`.harness/microdecisoes.md`), agora também consumido por `session/resume_context.py` (021).
...
- `021-hook-busca-ancorada/` 🟢 **(NOVO — mais recente)** — `cmd resume` no Claude passa a anexar o índice `.harness/microdecisoes.md` ao contexto reinjetado, ancorando a busca do agente antes de varreduras amplas; `resume_context.build_decisions_appendix` (novo, agnóstico ao harness) e campo `SessionSection.inject_decisions_index` (default `True`).
...
- 🟡 **Contagem de 110 testes está obsoleta:** a suíte cresceu para (segundo relato da sessão de fechamento da 021, não recontado por este Scout) 256 testes. O salto reflete acumulação orgânica ao longo de 010-021, não uma feature isolada.
