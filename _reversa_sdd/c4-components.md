...
> **Reconciliação de 2026-08-11** (pós-features 024-027): novo pacote `core/progress` (✨f026/f027: `service`/`stages`/`render`/`kanban`); `close_flow` com oferta consentida e tri-estado de versionamento (✨f024); `CommandService.execute_command(..., versionar_estado=True)` (✨f024); ramo `decisions --gate` da CLI virou advisory (✨f025); `config.py` com `ProgressSection`/`ProgressKanbanSection`, versão 2.5.0.
> **Reconciliação de 2026-08-11-b** (pós-feature 028): **nenhum componente novo** — a feature vive em métodos de componentes existentes: `DecisionService.compile_compact_view` (+ auxiliares `_extract_title`/`_write_if_changed`); `resume_context.build_decisions_appendix(..., compact_file=None)` com precedência compacta→índice; `_ensure_decisions_guidance` no `bootstrap/init_service.py`; `DecisionsSection.compact_file`/`compact_index_size` em `config.py` (versão 2.6.0). As duas bordas (CLI `decisions` e `_handle_stop` da ponte) derivam a compacta na mesma passada do índice.
...
graph TB
    subgraph drivers [Drivers de entrada]
        CLI["main.py<br/>[CLI argparse]<br/>13 subcomandos · resolve sink · serve HTTP · agy-hook ✨f009 · migrate ✨f020 · progress ✨f026"]
    ...
    subgraph core [Núcleo da Aplicação src/core/]
        ...
        subgraph services [Serviços de capacidade]
            ...
            ProgServ["core/progress ✨f026/f027<br/>ProgressService.measure (leitura pura, 5 fontes) · stages (paridade c/ skill) · render (markdown/JSON) · kanban (schema do board confinado)"]
        end
    end
    ...
    CLI -->|decisions --gate ✨f022, advisory desde ✨f025| Gate
    AGY -->|advisory via gate_evaluator ✨f022| Gate
    CLI -->|progress ✨f026| ProgServ
    ProgServ -->|reavalia gate em leitura pura, sem persistir| Gate
    ProgServ --> FsPort
    ProgServ --> GitPort
    Gate --> GitPort
    ResumeCtx --> FsPort
...
- **DecisionService** — `load_decisions`, `validate_integrity` (auto-relação e aresta órfã), `compile_index` (backlinks por verbos inversos). Caminhos vêm por parâmetro ✨f005. **✨f028:** `compile_compact_view(decisions, output_filepath, index_file, decisions_dir, max_items)` deriva a visão compacta `.harness/decisoes-recentes.md` (contagem, ponteiros, K títulos mais recentes por ID decrescente; `max_items=0` degrada para cabeçalho + contagem + ponteiros) na mesma passada do índice; auxiliares compartilhados `_extract_title` (H1 com fallback no ID) e `_write_if_changed` (usado nas duas escritas — índice e compacta —, mtime imóvel quando nada muda).
- **decisions/gate ✨f022/f023 (NOVO)** — `evaluate_registration_gate(git, repo_path, session, config) -> GateVerdict`: avaliação **pura** de pendência de registro de microdecisão (diff da âncora ∪ sujos, sem ficha `MD-*.md` tocada → pendente; fail-open barulhento). Duas identidades anti-loop: `compute_fingerprint` (fina, `sha1(âncora+HEAD+sujos)` — portão do encerramento, trabalho novo rearma) e `compute_lembrete_fingerprint` (grossa, `sha1(âncora)` ✨f023 — lembrete do Stop, máx. 1 por sessão). Consumido por três bordas — desde ✨f025, com **duas políticas**: `CloseFlow` (bloqueio + escape `--sem-decisao`, única política dura) e advisory nos fins de turno tanto na CLI `decisions --gate` (hook Stop do Claude: `Aviso:` em stderr, stdout sempre vazio, o JSON de soft-block foi aposentado; identidade grossa segue limitando a um aviso por sessão) quanto no `AntigravityHookBridge` (stderr via `gate_evaluator` injetado). Agnóstico ao harness (RN-N5).
- **CommandService** — slash commands agnósticos à IDE; âncora Git no `resume`; distingue ausente (`None`) de malformado (`MalformedSessionStateError`). Não conhece harness (RN-N5). **✨f018: consumido por `session/close_flow`**, que orquestra em volta dele (pré-check + ofertas), sem alterar seu contrato. **✨f024:** `execute_command(..., versionar_estado: bool = True)` — com `False`, fecha o estado no arquivo mas pula `commit_paths` e grava linha declarativa na narrativa; o MCP mantém `True` (D-04).
...
- **core/progress ✨f026/f027 (NOVO)** — quatro módulos: `service.py` (`ProgressService.measure() -> Medicao`, **leitura pura** pinada por teste, agrega cinco fontes: ciclo forward por artefatos físicos, regression-watch com a marca "pendência de reconciliação" virando alerta média, microdecisões com o gate reavaliado sem persistir fingerprint, estado de sessão e — com opt-in — cards manuais do board como `Medicao.demandas`); `stages.py` (tabela de estágio físico + `contar_checkboxes`/`listar_acoes` com o MESMO critério de linha — paridade com o skill `reversa-requirements`); `render.py` (markdown sem valor volátil + JSON com `aferido_em` opcional); `kanban.py` (✨f027, único módulo do core que conhece o schema do fork — `extrair_manuais` por posse de namespace `category == "harness"` e `render_board` determinístico sem consultar a hora). A borda CLI (`main.py`, ramo `progress`) concentra toda escrita: modo padrão regrava `.harness/progresso.md` (e o board com opt-in) atômico e write-only-when-changed; `--json` só stdout; `--em-hook` exit 1 apenas por artefato defasado; fonte ilegível → exit 2 sem regravar nada.
...
- **session/close_flow ✨f018 (NOVO)** — `SessionCloseFlow.run(repo_path, config, ..., sem_decisao=False) -> int`: fonte única da orquestração de `encerrar-sessao`, consumida pela CLI e pelos scripts finos da skill. Sequência: pré-check de pendência (`pending_work_paths`, restrito ao `session_file` desde ✨f019; **✨f024: virou oferta consentida** — `conduct_commit_pendente` devolve `bool`, TTY pergunta, sem TTY exige `--com-pendencias`) → gate de narrativa viva → **3º portão de registro de microdecisões (✨f022: marker `DECISAO_PENDENTE`, anti-loop por fingerprint fino, escape `--sem-decisao` com rastro na narrativa)** → **resolução do consentimento tri-estado `versionar_encerramento` (✨f024: TTY `[S/n]` default afirmativo; sem TTY silêncio é recusa, flags `--com-commit-encerramento` ⊻ `--sem-commit-encerramento`)** → `CommandService.execute_command("encerrar-sessao", versionar_estado=...)` → marker `ENCERRAMENTO_NAO_VERSIONADO` com `motivo` quando não versionou (✨f024, antes da oferta de push) → ofertas de fim de sessão (push → upgrade, via `EndSessionOffersService`). I/O injetável (markers sem TTY, perguntas com TTY).
- **session/resume_context ✨f021 (revisto ✨f028)** — `build_decisions_appendix(fs, index_file, enabled, compact_file=None) -> str`: função pura que compõe o apêndice de decisões anexado ao `cmd resume`; o gate (`active_harness == "claude" and inject_decisions_index`) é calculado na borda (`main.py`) e passado como parâmetro. **✨f028:** precedência compacta→índice — injeta `.harness/decisoes-recentes.md` quando existe; ausente, cai para o índice completo com `Aviso:` em stderr (autoresolvente na próxima compilação); ambos ausentes → string vazia, só o estado é reinjetado.
...
- **config.py** — `HarnessConfig` (`[harness]/[formatting]/[sync]/[decisions]/[session]`) e `load_config(fs)`. ✨ `[decisions]` é a chave da feature 005; ✨ `SessionSection` (`[session].state_file`, default `.harness/estado-da-sessao.md`) é a chave da feature 006, fonte única do caminho de sessão lida por CLI e MCP. **✨f022:** `DecisionsSection.require_registration` (default `True`) liga o gate de registro. **✨f026/f027:** `ProgressSection` (`[progress].file`, default `.harness/progresso.md`) com `ProgressKanbanSection` aninhada (`[progress.kanban].enabled` default `False`, `file` default `.vscode/vscode-kanban.json`). **✨f028:** `DecisionsSection.compact_file` (default `.harness/decisoes-recentes.md`) e `compact_index_size` (int, `ge=0`, default 10); **literal de versão em `2.6.0`**.
- **cache.py** — `SyncCache`.
