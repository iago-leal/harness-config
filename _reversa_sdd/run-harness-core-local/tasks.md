...
- [x] Python 3 e venv configurados localmente em `.harness/harness-core/.venv`.
...
- [x] T-01, Criar o script wrapper de conveniência Bash `harness` na raiz do projeto 🟢
  ...
  - Critério de pronto: Executar `./harness` na raiz encaminha a chamada para `.harness/harness-core/src/main.py` usando `.harness/harness-core/.venv/bin/python3`.
...
- [x] T-02, Adicionar fail-fast e instruções para venv ausente 🟢
  ...
  - Critério de pronto: Ao deletar ou renomear temporariamente a pasta `.harness/harness-core/.venv`, executar `./harness` falha com código 1, escrevendo erro instrutivo no stderr.
  - Confiança: 🟢 CONFIRMADO
...
- [x] TT-01, Teste do happy path do wrapper 🟢
  - Origem no legado: `.harness/harness-core/tests/test_wrapper.py:4-19` (`test_wrapper_help`)
  - Critério de pronto: Rodar `PYTHONPATH=harness-core .harness/harness-core/.venv/bin/pytest .harness/harness-core/tests/test_wrapper.py` e validar se a chamada com `--help` retorna código de saída 0 e imprime o texto correto de ajuda.
  - Confiança: 🟢 CONFIRMADO

- [x] TT-02, Teste do happy path para comandos específicos do core 🟢
  - Origem no legado: `.harness/harness-core/tests/test_wrapper.py:21-33` (`test_wrapper_cmd_clarificar`)
...
4. Implementar testes automatizados em `.harness/harness-core/tests/test_wrapper.py`.
