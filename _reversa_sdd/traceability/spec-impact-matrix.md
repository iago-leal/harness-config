...
| **InitService** ✨                  | `core/bootstrap/init_service.py`      | RN-N19, RN-N20 (estendidas na f009); RN-N26                                                                                                 | **007**, **009**       | 0014, 0016                    | —                                                    | **HIGH**   |
...
| **CLI driver (`main.py`)**          | `src/main.py`                         | RN-08, RN-N9, RN-N11, RN-N21; orquestra sinks; subcomando `agy-hook <evento>` (f009)                                                        | 001..009               | 0007, 0013, 0014, 0015, 0016  | T3, T5 e T6 fechados                                 | **HIGH**   |
...
> Situação dos bugs nesta atualização: **Todos os achados históricos (T1 ao T6) estão totalmente resolvidos** no HEAD (T1/T3 no fix de drivers; T2/T5 na feature 006; T4/T6 na feature 008). A feature 009 não introduziu bug latente; resta apenas a nota de manutenção do `command` absoluto no `hooks.json` (mitigada por `upgrade`). Nenhuma dívida técnica está em aberto atualmente.
