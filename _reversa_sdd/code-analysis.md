...
> **Reconciliação de 2026-08-11-b** (Archaeologist, pós-feature 028 — delta lido da árvore de trabalho, sem commit): **4. `core/decisions`** ganhou a **visão compacta** (`compile_compact_view` + auxiliares `_extract_title`/`_write_if_changed`; `compile_index` refatorado sobre os mesmos auxiliares — write-only-when-changed nas DUAS visões); **8. `core/session`** teve `resume_context.py` reescrito com precedência **compacta→índice** (`build_decisions_appendix` ganhou `compact_file`); **1. `core/bootstrap`** ganhou o passo 10 do init (`_ensure_decisions_guidance` — trecho de guidance idempotente por marcador no arquivo da engine); as bordas `main.py` (ramo `decisions` + avisos do `cmd resume`) e `hook_bridge._handle_stop` derivam a compacta na MESMA passada do índice; `config.py` ganhou `DecisionsSection.compact_file`/`compact_index_size`. Core 2.5.0 → **2.6.0**; suíte 372 → 389.
...
**Arquivos:** `src/core/bootstrap/service.py` (57 linhas), `src/core/bootstrap/init_service.py` (368 linhas desde a 028; a descrição do `init` abaixo antecede a fonte única da 020 e permanece defasada nos passos de cópia — ver nota de defasagem estrutural).
...
- **`init_target(fs, process, target_path, active_harness, upstream_path, version)`**:
  ...
  7. **(feature 028, NOVO)** Último passo do `initialize_project`: `_ensure_decisions_guidance(target_path, active_harness)` grava um **trecho de guidance sobre microdecisões** no arquivo da engine (mapa `_GUIDANCE_FILE_BY_HARNESS`: `claude→CLAUDE.md`, `antigravity→AGENTS.md`, `gemini→GEMINI.md`; harness desconhecido → no-op). **Idempotência por marcador** `DECISIONS_GUIDANCE_MARKER = "<!-- harness:decisoes -->"`: arquivo ausente → cria só com o trecho; marcador já presente → retorna sem tocar; arquivo presente sem marcador → **append** (`conteúdo.rstrip("\n") + "\n\n" + trecho`, preservando o texto do usuário), sempre via `write_file_atomic`. O `upgrade` **nunca** toca esse arquivo (write-once do init). Risco aceito por design: usuário que remove o marcador mas mantém o texto terá duplicação num re-init. 🟢
...
**Arquivo:** `src/core/decisions/service.py` (195 linhas desde a 028). Quatro responsabilidades:
...
- Título de cada item extraído do H1 `# MD-XXXX — <título>` por regex — desde a 028, num **único ponto compartilhado**: o método estático `_extract_title(fs, decision)` (regex do H1, fallback para o ID), usado pelas duas visões.
- Sub-linha `↳ <saídas> · <entradas>` montada por composição.
- Cabeçalho opcional concatenado no topo. Gravação **atômica** e, desde a 028, **condicionada a mudança**: o auxiliar `_write_if_changed(filepath, content)` compara o conteúdo novo com o existente e só grava quando os bytes diferem (mtime imóvel sem alteração — RN de artefato derivado estável, alinhada ao `progresso.md` da 026).

**`compile_compact_view(decisions, output_filepath, index_file, decisions_dir, max_items)` (feature 028, NOVO):** deriva a **visão compacta** (`.harness/decisoes-recentes.md`) na MESMA passada do índice, em todas as bordas que indexam (CLI, ponte Antigravity e, desde a MD-0023/core 2.6.1, a tool MCP `process_decisions` — fix do G-20/T8). Formato: cabeçalho `# Decisões recentes` declarando a natureza derivada, três linhas de orientação (ponteiros para o índice completo, a pasta de fichas e o comando `./harness decisions`), `Total: N ficha(s)` e as **K mais recentes por ID decrescente** (`max_items`, default 10; `0` degrada para cabeçalho+contagem+ponteiros), cada uma como `- **MD-NNNN** — título` (sem backlinks). Reusa `_extract_title` e `_write_if_changed`; é chamada **após** `compile_index`, portanto nunca antes da validação de integridade.
...
### `resume_context.py` — apêndice de decisões no resume (feature 021, revisto na 028) 🟢

Função pura `build_decisions_appendix(fs, index_file, enabled, compact_file=None) -> str` (47 linhas no módulo), agnóstica ao harness (RN-N5): não decide o gate, apenas o executa. Retorna `""` (não-bloqueante, RN-N4) se `enabled` for falso ou se nenhuma das fontes existir/tiver conteúdo. **Precedência desde a 028 (MD-0022):** com `compact_file` presente e não vazio, injeta a **visão compacta** sob o cabeçalho `"\n\n---\n## Decisões recentes (índice completo sob demanda)\n\n"`; compacta ausente/vazia → **fallback** para o índice completo sob o cabeçalho antigo (`## Índice de decisões (consulte antes de buscas amplas)`); ambos ausentes → `""`. Sem `compact_file` (chamadores antigos), o comportamento 021 permanece byte-idêntico.

Fiação em `main.py` (ramo `cmd resume`, só após `execute_command` produzir o corpo da narrativa): `enabled = config.harness.active_harness == "claude" and config.session.inject_decisions_index` — o gate por harness (**Claude-first**; Gemini/Antigravity adiados, decisão explícita da feature 021) e o flag de opt-out (`SessionSection.inject_decisions_index`, default `True`, seção `[session]` do `harness.toml`) vivem na borda; a função em si é composição pura. **Avisos de borda (028):** compacta ausente com índice presente → `Aviso:` em stderr anunciando o fallback autoresolvente ("rode ./harness decisions ou aguarde o próximo fim de turno"); ambos ausentes → aviso de que só o estado será injetado. Exit 0 em todos os casos. O apêndice é **anexado depois** do estado da sessão no `result_msg`, de modo que, sob truncamento por teto de tamanho do sink (`HookContextSink`, RN-N8), o estado tem precedência e o apêndice cede.

**Decisão de escopo (D-02 da 021, refinada pela 028/MD-0022):** a 021 injetava o **índice** (~1,7 KB à época) para não estourar o teto de 10 KB do sink com a pasta `decisoes/` inteira; a 028 constata que o próprio índice incha linearmente com as fichas (22 e crescendo, com backlinks) e o substitui na injeção pela **visão compacta de custo ~constante** (contagem + ponteiros + K=10 títulos). O índice completo permanece derivado e a um passo de leitura, sob demanda; as fichas `MD-NNNN` seguem como aprofundamento via ponteiros.
...
**Delta 028:** `DecisionsSection` ganhou `compact_file: str = ".harness/decisoes-recentes.md"` e `compact_index_size: int = 10` com `ge=0` (negativo → erro de validação Pydantic barulhento, RN-N4; `0` degrada a compacta para cabeçalho+contagem+ponteiros). Literal de versão em **`2.6.0`**.
...
- **`main.py`** (driver CLI v2.6.0): Argparse expandido para expor os subcomandos `init` (inicialização de workspace físico no destino), `upgrade` (atualização evolucionária não destrutiva) e **`agy-hook <evento>`** (feature 009). Incorpora alertas passivos no topo do boot de comandos da CLI.
  ...
  - **Ramo `decisions` (revisto na 028):** após `compile_index`, chama `service.compile_compact_view(decisions, config.decisions.compact_file, output_file, decisoes_dir, config.decisions.compact_index_size)` e informa "Visão compacta derivada em '...'" — as duas visões saem da MESMA passada, ambas write-only-when-changed. O modo `--gate` mantém o contrato 025 (advisory puro), agora também derivando a compacta.
...
- **`stop` (decisões) → `_handle_stop`:** carrega as microdecisões de `decisions_dir`, roda `validate_integrity` (erros são **logados**, nunca bloqueiam nem reentram no laço), `compile_index(decisions, decisions_index_file, decisions_header_file)` e, desde a 028, `compile_compact_view(decisions, decisions_compact_file, decisions_index_file, decisions_dir, decisions_compact_size)` — equivale a `./harness decisions` (as duas visões na mesma passada). Stdout: `{}` (**nunca** `{"decision": "continue"}`).
