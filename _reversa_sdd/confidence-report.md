...
- **T7 (NOVO, 2026-07-05, EM ABERTO):** o servidor MCP grava o cache de sincronia em `.harness/sync_cache.json` (underscore, chumbado em `server.py:42`); a CLI usa `.harness/sync-cache.json` (hífen), o mesmo nome que o `.gitignore` do `init` cobre. Consequência: o cache do MCP escapa do `.gitignore` e, desde a feature 019 (pré-check de pendência não mascara mais `.harness/` inteiro), seria oferecido para commit por engano. **Não é uma lacuna de especificação** — é um bug real, pequeno e com fix óbvio (alinhar os dois literais), documentado em `architecture.md` §5, `erd-complete.md` §3 e `c4-containers.md`. Não requer decisão do usuário, só execução do fix numa próxima feature.
