...
def test_extract_commands_falls_back_to_parent_help():
    # Subparser criado só com help= (como no main.py real): o gerador deve usar
    # esse texto, não "Sem descrição.".
    parser = argparse.ArgumentParser()
    subparsers = parser.add_subparsers(dest="command")
    subparsers.add_parser("instalar", help="Ajuda via help do pai")

    service = DocumentationService(MockFileSystem())
    commands = service.extract_commands(parser)

    assert len(commands) == 1
    assert commands[0]["name"] == "instalar"
    assert commands[0]["help"] == "Ajuda via help do pai"


def test_extract_commands_real_cli_has_no_empty_help():
    # Nenhum comando do CLI real pode cair em "Sem descrição.".
    service = DocumentationService(MockFileSystem())
    by_name = {c["name"]: c["help"] for c in service.extract_commands(build_parser())}
    assert "install-prompt" in by_name
    for name, help_text in by_name.items():
        assert help_text != "Sem descrição.", f"comando '{name}' ficou sem ajuda"
