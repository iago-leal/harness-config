...
_CLAUDE_SKILL = "proj/.claude/skills/encerrar-sessao/SKILL.md"
_AGY_SKILL = "proj/.agents/skills/encerrar-sessao/SKILL.md"


def test_apply_local_materializers_claude_writes_skill_and_settings():
    """Para o Claude: a skill de sessão (Claude + Antigravity) E o
    ...
    assert _CLAUDE_SKILL in fs.written_files
    assert _AGY_SKILL in fs.written_files
...
def test_apply_local_materializers_antigravity_also_writes_hooks_json():
    """Para harness Antigravity, além da skill de sessão, materializa o
...
def test_apply_local_materializers_migra_artefatos_antigos():
    """feature 018: num projeto já instalado (workflow/command .md antigos),
    materializar grava a skill, remove os órfãos legados e preserva o workflow de
    terceiro no mesmo diretório (migração via upgrade, não-destrutiva).
    """
    fs = MockFileSystem()
    stale_cmd = "proj/.claude/commands/encerrar-sessao.md"
    stale_wf = "proj/.agent/workflows/encerrar-sessao.md"
    terceiro = "proj/.agent/workflows/outro-workflow.md"
    fs.existing_files.update({stale_cmd, stale_wf, terceiro})
    fs.written_files[stale_cmd] = "command antigo"
    fs.written_files[stale_wf] = "workflow antigo"
    ...
    assert _CLAUDE_SKILL in fs.written_files
    assert _AGY_SKILL in fs.written_files
    assert not fs.exists(stale_cmd)
    assert not fs.exists(stale_wf)
