...
- **`user-stories/fluxo-de-sincronia-e-sessao.md`** e **`flowcharts/*.md`** (5 arquivos) datam de **2026-06-23** e descrevem o **legado purgado** (`sync-check.sh`, `format-on-edit.sh`, `gerar-index-decisoes.sh`, `.claude/ESTADO-DA-SESSAO.md`, manifesto `pyproject.toml`). Contradizem o estado atual (Python, `.harness/`). → `questions.md#5`. **Fora do conjunto regenerado hoje; reportado, não corrigido pelo Revisor.**
